...
def makeWorld():
    ...
    while i < len(obstacleList):
        ...
        i = j + 1
        file.write('    Solid {\n')
        file.write('      translation ' + str(int(obsTileNum % numCol) * 1.0) + ' 0.2 ' + str(int(obsTileNum / numCol) * 1.0) + '\n')
        file.write('      scale 4.3 4.3 4.3\n      children [\n        Shape {\n          appearance Appearance {\n            material Material {\n              diffuseColor 0.2 0.2 0.8\n            }\n          }\n          geometry Cylinder {\n            height 0.1\n            radius 0.05\n          }\n        }\n      ]\n')
    ...
    while i < len(speedbumpList):
        ...
        file.write('      translation ' + str(int(sbNum % numCol) * 1) + ' 0 ' + str(int(sbNum / numCol) * 1) + '\n')
        #file.write('      scale 3.3 3.3 3.3\n')
    ...
    while i < len(seesawList):
        ...
        i = j + 3
        file.write('    seesaw {\n')
        file.write('      translation ' + str(int(ssNum % numCol) * 1.0) + ' 0.2 ' + str(int(ssNum / numCol) * 1.0) + '\n')
    ...
    file.write(' ' + str(numCol / 2 * 1.0 - 0.15) + ' 0 ' + str(numRow / 2 * 1.0 - 0.15) + '\n')
    file.write('      boundingObject Plane {\n')
    file.write('        size ' + str(numCol * 1.0) + ' ' + str(numRow * 1.0) + '\n      }\n    }\n')
    ...
    if eEntrance != -1:
        ...
        if eExit - eEntrance == -3 * numCol + 1: #N
            file.write('      translation ' + str((eEntrance % numCol - 3) * 1.0) + ' 0 ' + str((int(eEntrance / numCol) - 3) * 1.0) + '\n')
    ...
    while i < len(checkpointList):
        ...
        if preEvac == 0 and i < len(checkpointList):
            ...
            if checkInd <= evacEntrance and pathList.index(int(checkpointList[i:k])) > evacEntrance:
                preEvac = preEvac + 1
        file.write('    Solid {\n')
        file.write('      translation ' + str(int(checkpointNum % numCol) * 1.0 - 0.12) + ' 0 ' + str(int(checkpointNum / numCol) * 1.0 - 0.12) + '\n')
